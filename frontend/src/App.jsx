...
function App() {
  // Preset bbox provinsi Kalimantan (perkiraan)
  const KALIMANTAN_PROV_BBOX = {
    Semua: { minLat: -4.5, maxLat: 4.5, minLon: 108.5, maxLon: 119.8 },
    Kalbar: { minLat: -2.5, maxLat: 2.5, minLon: 108.0, maxLon: 113.5 },
    Kalteng: { minLat: -3.5, maxLat: 1.5, minLon: 111.0, maxLon: 115.5 },
    Kalsel: { minLat: -4.5, maxLat: -1.5, minLon: 113.0, maxLon: 116.8 },
    Kaltim: { minLat: -2.8, maxLat: 2.8, minLon: 115.0, maxLon: 119.8 },
    Kalut: { minLat: 1.5, maxLat: 4.8, minLon: 115.5, maxLon: 118.8 },
  };
  ...
  // Ambil data backend (pakai bbox)
  useEffect(() => {
    setLoading(true);
    axios
      axios.get(`${API_BASE}/api/pembangkit`, { params: bbox })
      .then((res) => {
          ...
          <div className="bg-white rounded shadow-lg p-1 flex flex-col gap-1">
            ...
            <button
              ...
              className={`p-2 rounded hover:bg-gray-100 ${
                basemap === "satellite" ? "bg-slate-200" : ""
              }`}
              title="Satellite"
            >
              🛰️
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full overflow-hidden animate-fade-in-up">
            <div className="bg-emerald-600 p-4 flex justify-between items-center">
              ...
              <button
                ...
                className="text-white hover:text-emerald-200 text-xl font-bold"
              >
                ×
              </button>
            </div>
